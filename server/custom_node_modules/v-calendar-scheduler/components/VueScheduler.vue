  ...
  methods: {
    openEventDialog(data) {
      if (!this.disableDialog) {
        ...
        if (data instanceof Date) {
          config.date = data;
          config.startTime = moment().format("HH:mm a");
        } else {
    ...
    bindEvents() {
      ...
      EventBus.$on("event-clicked", event => {
        ...
        const { fields } = this.eventDialogConfig;
        var temp = event._e;
        fields[0].value = temp.header;
        fields[1].value = temp.body;
        temp["isedit"] = true;
        EventDialog.show(temp, fields)
          .$on("event-deleted", event => {
            this.$emit("event-deleted", event._e, temp.id);
          })
          .$on("event-created", event => {
            this.$emit("event-updated", event._e, temp.id);
          });
      });
    },
    goToToday() {
